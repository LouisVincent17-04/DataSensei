...
class CodingQuizController extends Controller
{
    ...
    public function show(string $slug, Challenge|int $challenge)
    {
        ...
        // ── Build attempt data ────────────────────────────────────────────
        // Rule:
        //   • Already passed      → no attempt needed, no timer
        //   • Existing DB attempt → restore it (clock was already running)
        //   • First unsolved      → create attempt NOW (stamp started_at)
        //   • Later unsolved      → has_attempt=false, clock starts lazily via start()
        $attempts            = [];
        ...
        return view('student.coding-challenge-quiz', compact(
            'slug', 'challenge', 'priorSubmissions', 'attempts'
        ));
    }
