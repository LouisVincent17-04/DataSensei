...
/**
 * Seeds the challenge_categories table with coding-challenge-specific entries.
 *
 * Your existing MCQ challenges share the same challenge_categories table.
 * Coding challenges also reference challenge_categories via challenges.challenge_category_id,
 * so we reuse the same 5 rows (Newbie → Professional) already in your DB.
 *
 * Run this ONLY if your challenge_categories table is empty.
 * If it already has data from your SQL dump, SKIP this seeder.
 */
class CodingChallengeCategorySeeder extends Seeder
{
    public function run(): void
    {
        // Guard: skip if categories already exist
        if (DB::table('challenge_categories')->exists()) {
            $this->command->info('challenge_categories already seeded — skipping.');
            return;
        }
        ...
        $this->command->info('challenge_categories seeded.');
    }
}