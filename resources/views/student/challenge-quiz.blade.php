...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .page-quiz-header { position: sticky; top: 0; z-index: 100; background: rgba(13, 19, 32, 0.96); backdrop-filter: blur(10px); border-bottom: 1px solid var(--border); padding: 14px 32px; display: flex; align-items: center; justify-content: space-between; gap: 16px; }
    .page-quiz-breadcrumb { font-size: 0.7rem; color: var(--dim); text-transform: uppercase; letter-spacing: 0.08em; font-weight: 700; margin-bottom: 3px; }
    ...
    .page-quiz-title { font-size: 1.125rem; font-weight: 800; color: var(--text); }
    ...
    .page-quiz-progress-label { font-size: 0.7rem; color: var(--muted); font-weight: 700; white-space: nowrap; }
    .page-quiz-progress-track { width: 120px; height: 5px; background: var(--surface2); border-radius: 10px; overflow: hidden; }
    .page-quiz-progress-fill { height: 100%; background: linear-gradient(90deg, var(--accent), var(--accent3)); border-radius: 10px; transition: width 0.3s ease; }
    .page-quiz-timer { font-size: 1.375rem; font-weight: 800; font-variant-numeric: tabular-nums; display: flex; align-items: center; gap: 8px; color: var(--accent); background: var(--surface); padding: 8px 18px; border-radius: 8px; border: 1px solid var(--border); white-space: nowrap; }
    .page-quiz-timer.urgent { color: var(--warn); border-color: rgba(239,68,68,0.4); animation: blink 1s step-end infinite; }
    @keyframes blink { 0%,100%{opacity:1} 50%{opacity:0.45} }
    .page-quiz-body { max-width: 820px; width: 100%; margin: 36px auto; padding: 0 32px 80px; }
    .attempt-notice { background: linear-gradient(135deg, rgba(59,130,246,.12), rgba(16,185,129,.08)); border: 1px solid rgba(59,130,246,.25); border-radius: var(--radius); padding: 16px 20px; margin-bottom: 18px; display: flex; gap: 14px; align-items: flex-start; }
    .attempt-code { width: 42px; height: 42px; border-radius: 10px; background: rgba(59,130,246,.13); border: 1px solid rgba(59,130,246,.28); display:flex; align-items:center; justify-content:center; font-weight:900; color:#bfdbfe; letter-spacing:.04em; flex-shrink:0; }
    .attempt-notice strong { display:block; margin-bottom: 4px; font-size:.95rem; }
    .attempt-notice p { color: var(--muted); font-size:.84rem; line-height:1.55; }
    .attempt-notice.practice { background: linear-gradient(135deg, rgba(245,158,11,.12), rgba(59,130,246,.06)); border-color: rgba(245,158,11,.22); }
    .attempt-notice.practice .attempt-code { background: rgba(245,158,11,.12); border-color: rgba(245,158,11,.24); color:#fcd34d; }
    .save-state { color: var(--dim); font-size:.75rem; margin-top: 8px; }
    .save-state.saved { color: var(--accent3); }
    .save-state.error { color: var(--warning); }
    .page-quiz-info-bar { background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); padding: 18px 24px; margin-bottom: 32px; display: flex; justify-content: space-between; align-items: center; gap: 16px; flex-wrap: wrap; }
    ...
    .page-quiz-meta-pill { font-size: 0.75rem; font-weight: 800; padding: 4px 10px; border-radius: 20px; }
    ...
    .page-quiz-question-card { background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); padding: 28px; margin-bottom: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.2); transition: border-color 0.2s, transform .15s; }
    .page-quiz-question-card.answered { border-color: rgba(16,185,129,.36); }
    .page-quiz-q-header { display: flex; align-items: flex-start; gap: 14px; margin-bottom: 20px; }
    .page-quiz-q-number { width: 28px; height: 28px; border-radius: 6px; background: var(--surface2); border: 1px solid var(--border); display: flex; align-items: center; justify-content: center; font-size: 0.75rem; font-weight: 800; color: var(--muted); flex-shrink: 0; margin-top: 2px; }
    .page-quiz-question-text { font-size: 1rem; font-weight: 700; line-height: 1.55; color: var(--text); white-space: pre-wrap; }
    .page-quiz-option-label { display: flex; align-items: center; gap: 14px; padding: 14px 18px; border: 1px solid var(--border); border-radius: 8px; cursor: pointer; transition: all 0.18s; margin-bottom: 10px; background: var(--bg); }
    ...
    .page-quiz-option-label.selected span { color: var(--text); font-weight: 600; }
    .page-quiz-submit-bar { background: var(--surface); padding: 20px 28px; border-radius: var(--radius); border: 1px solid var(--border); display: flex; justify-content: space-between; align-items: center; gap: 16px; margin-top: 36px; box-shadow: 0 8px 24px rgba(0,0,0,0.3); }
    ...
    .page-quiz-submit-btn { background: var(--accent); color: #fff; border: none; padding: 13px 28px; font-size: 0.9375rem; font-weight: 800; border-radius: var(--radius-sm); cursor: pointer; transition: all 0.2s; box-shadow: 0 4px 14px rgba(59,130,246,0.35); white-space: nowrap; font-family: inherit; display: flex; align-items: center; gap: 8px; }
    .page-quiz-submit-btn:hover { background: var(--accent-hover); transform: translateY(-1px); }
    .page-quiz-submit-btn:disabled { opacity:.65; cursor:not-allowed; transform:none; }
    @media (max-width: 768px) { .page-quiz-header { flex-direction: column; align-items: flex-start; gap: 12px; padding: 12px 16px; } .page-quiz-body { padding: 0 16px 60px; } .page-quiz-submit-bar { flex-direction: column; align-items: stretch; } .page-quiz-progress-track { width: 80px; } }
  </style>
</head>
<body>
