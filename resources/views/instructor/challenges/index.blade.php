...
<div class="layout">
  ...
  <main class="main">
    ...
    <div class="top">
      <div>
        <h1 class="title">Challenge Pool</h1>
        <p class="subtitle">View all MCQ and coding challenges currently available in the DataSensei challenge paths.</p>
      </div>
    </div>
    ...
    <div class="card table-wrap">
      <table class="table">
        <thead><tr><th>Title</th><th>Category</th><th>Type</th><th>Items</th><th>XP</th></tr></thead>
        <tbody>
        @forelse($challenges as $challenge)
          <tr>
            <td>{{ $challenge->title }}</td>
            <td>{{ $challenge->category->name ?? '—' }}</td>
            <td><span class="badge {{ $challenge->is_coding_challenge ? 'warn' : 'good' }}">{{ $challenge->is_coding_challenge ? 'Coding' : 'MCQ' }}</span></td>
            <td>{{ $challenge->is_coding_challenge ? $challenge->coding_questions_count : $challenge->questions_count }}</td>
            <td>{{ $challenge->base_xp }}</td>
          </tr>
        @empty
          <tr><td colspan="5" class="muted">No challenges found.</td></tr>
        @endforelse
        </tbody>
      </table>
