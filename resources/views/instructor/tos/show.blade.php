...
<head>
  ...
  <title>{{ $tos->title }} — DataSensei</title>
...
<div class="layout">
  ...
  <main class="main">
    ...
    <div class="top">
      <div>
        <h1 class="title">{{ $tos->title }}</h1>
        <div class="subtitle">{{ $tos->classRoom->name ?? 'Template' }} · Module {{ $tos->module_no }}</div>
      </div>
    ...
    <div class="card table-wrap">
      <table class="table">
        <thead><tr><th>ILO / Topic</th><th>Difficulty</th><th>Cognitive Level</th><th>Items</th><th>Update</th></tr></thead>
        <tbody>
        @forelse($tos->rows as $row)
          <tr>
            <td>{{ $row->ilo->ilo_code ?? 'General' }} — {{ $row->topic_title }}</td>
            ...
            <td>
              <form id="row-{{ $row->id }}" method="POST" action="{{ route('instructor.tos.rows.update', [$tos, $row]) }}">
                @csrf
                @method('PATCH')
                <input class="input" name="cognitive_level" value="{{ $row->cognitive_level }}">
              </form>
            </td>
            <td><input form="row-{{ $row->id }}" class="input" type="number" min="0" max="200" name="item_count" value="{{ $row->item_count }}" style="width:90px"></td>
            <td><button form="row-{{ $row->id }}" class="btn secondary" type="submit">Save</button></td>
          </tr>
        @empty
          <tr><td colspan="5" class="muted">No TOS rows found.</td></tr>
        @endforelse
        </tbody>
      </table>
    </div>
