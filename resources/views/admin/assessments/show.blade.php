...
@section('eyebrow', 'Assessment Content')
        ...
        <details class="management-item" @if($loop->first) open @endif>
          ...
          <div class="management-editor">
            ...
            @if($question->question_type === 'mcq')
              <ol style="margin:14px 0 0 22px;color:var(--muted);line-height:1.8">@foreach($question->options as $option)<li style="color:{{ $option->is_correct ? '#a7f3d0' : 'var(--muted)' }}">{{ $option->option_text }} @if($option->is_correct)<strong> — Correct</strong>@endif</li>@endforeach</ol>
            @else
              <div style="margin-top:14px"><strong>Accepted answers:</strong><ul style="margin:8px 0 0 22px;color:var(--muted);line-height:1.8">@foreach($question->blankAnswers as $answer)<li>{{ $answer->answer_text }} {{ $answer->is_case_sensitive ? '(case-sensitive)' : '(case-insensitive)' }}</li>@endforeach</ul></div>
            @endif
            @if($question->explanation)<p style="margin-top:14px;color:var(--muted)"><strong style="color:var(--text)">Explanation:</strong> {{ $question->explanation }}</p>@endif
          </div>
        </details>
      @endforeach
    </div>
