...
<form method="POST" action="{{ $formAction }}" data-coding-form data-check-url="{{ route('admin.coding-challenges.check-tests') }}" data-locked="{{ $locked ? '1' : '0' }}">
  ...
  @if($locked)
    <div class="notice error">
      This challenge already has submission history. Challenge settings, problem titles and reference solutions may be corrected, but problem descriptions, starter code, limits and test cases are frozen. Create a new version to change them.
    </div>
  @endif
  ...
  <section class="panel">
    <div class="panel-head">
      <div class="panel-heading">
        ...
        <p class="panel-subtitle">The content code groups versions of the same challenge. Only one version per content code is available to learners at a time.</p>
      </div>
    </div>
      ...
      <div class="form-grid three">
        ...
        <div class="field">
          ...
          <select id="challenge-status" class="select" name="is_active">
            ...
            <option value="1" @selected((bool) old('is_active', $challenge->is_active))>Available to learners</option>
          </select>
        </div>
      </div>
    </div>
  ...
  <section class="panel">
    <div class="panel-head">
      <div class="panel-heading">
        ...
        <p class="panel-subtitle">The title and description learners see on the map, plus the challenge-level timer, XP and map order.</p>
      </div>
    </div>
    <div class="panel-body">
      <div class="form-grid three">
  ...
  <section class="panel" data-question-editor>
    <div class="panel-head">
      <div class="panel-heading">
        ...
        <p class="panel-subtitle">Learners solve the problems in order. Each problem is graded against its test cases: a case passes when the program's output matches the expected output exactly, ignoring trailing whitespace.</p>
      </div>
      @unless($locked)
      ...
      @foreach($questionRows as $row)
        ...
        <article class="question-item cc-question">
          ...
          <div class="cc-cases-head">
            ...
            <span class="dim">Visible cases are shown to learners as samples. Hidden cases are graded but never shown.</span>
          </div>
...
@include('admin.coding-challenges._preview')
</div>
...
@push('scripts')
<script src="{{ asset('js/admin-coding-challenge-editor.js') }}"></script>
@endpush
